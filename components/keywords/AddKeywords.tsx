      ...
      if (nkwrds.keywords) {
         ...
         if (!multiDevice && (keywordsArray.length === 1 || currentKeywords.length === keywordExist.length) && keywordExist.length > 0) {
            ...
            const newKeywords = keywordsArray.flatMap((k) =>
               devices
                  .filter((device) => !currentKeywords.includes(`${k}-${device}-${nkwrds.country}${nkwrds.city ? `-${nkwrds.city}` : ''}`))
                  .map((device) => ({
                     keyword: k,
                     device,
         ...
         <div data-testid="addkeywords_modal">
            <div>
                  ...
                  <div>
                     <SelectField
                        ...
                        options={Object.keys(countries).map((countryISO: string) => {
                           return { label: countries[countryISO][0], value: countryISO };
                        })}
               ...
               <div className="relative mt-2">
                  ...
                  />
                  <span className="absolute text-gray-400 top-2 left-2">
            ...
            <div className="mt-6 text-right text-sm font-semibold flex justify-between">
               ...
               <button className=" py-2 px-5 rounded cursor-pointer bg-blue-700 text-white" onClick={() => !isAdding && addKeywords()}>
