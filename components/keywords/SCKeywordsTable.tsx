   ...
   const SCCountryData: { [key: string]: SCCountryDataType } = useMemo(() => {
      ...
      Object.keys(finalKeywords).forEach((dateKey) => {
         finalKeywords[dateKey].forEach((keyword) => {
            ...
            if (!countryData[kCountry]) {
               countryData[kCountry] = { keywords: 0, impressions: 0, visits: 0 };
            }
               ...
               <div className="font-semibold text-sm py-4 px-8 text-gray-500 ">
                  <ul className="">
                     <li className="inline-block mr-4">
                        <a className="block px-2 py-2 cursor-pointer hover:text-indigo-600" onClick={() => addSCKeywordsToTracker()}>
            ...
            <div className="domkeywordsTable domkeywordsTable--sckeywords styled-scrollbar w-full overflow-auto min-h-[60vh]">
               <div className=" lg:min-w-[800px]">
                     ...
                     <span className="domKeywords_head_keyword flex-1 basis-20 w-auto ">
                        {finalKeywords[device].length > 0 && (
                           ...
                           ${selectedAllItems ? ' bg-blue-700 border-blue-700 text-white' : 'text-transparent'}`}
                              onClick={() =>
                                 setSelectedKeywords(selectedAllItems ? [] : finalKeywords[device].map((k: SearchAnalyticsItem) => k.uid))
                              }
                           >
                  ...
                  <div className="domKeywords_keywords border-gray-200 min-h-[55vh] relative" data-domain={domain?.domain}>
                     ...
                     {!isLoading && finalKeywords[device] && finalKeywords[device].length > 0 && (
                        <div
                           className={`domKeywords_head hidden lg:flex p-3 px-6 bg-[#FCFCFF]
                           text-gray-600 justify-between items-center font-semibold border-y`}
                        >
                           ...
                           <span className="domKeywords_head_imp flex-1 text-center">{formattedNum(viewSummary.impressions)}</span>
                           <span className="domKeywords_head_visits flex-1 text-center">{formattedNum(viewSummary.visits)}</span>
                     ...
                     {isConsoleIntegrated && isLoading && <p className=" p-9 pt-[10%] text-center text-gray-500">Loading Keywords...</p>}
                     {!isConsoleIntegrated && (
                        <p className=" p-9 pt-[10%] text-center text-gray-500">
                           Google Search Console has not been Integrated yet. Please follow{' '}
                           <a
                              className="text-indigo-600 underline"
                              href="https://docs.serpbear.com/miscellaneous/integrate-google-search-console"
                              target="_blank"
                              rel="noreferrer"
                           >
                              These Steps
