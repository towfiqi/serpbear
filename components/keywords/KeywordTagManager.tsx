...
const KeywordTagManager = ({ keyword, allTags = [], closeModal }: keywordTagManagerProps) => {
   ...
   return (
      <Modal
         ...
         title={`Tags for Keyword "${keyword && keyword.keyword}"`}
      >
         <div className="text-sm my-8 ">
            {keyword && keyword.tags.length > 0 && (
               <ul>
                  {keyword.tags.map((tag: string) => {
                     return (
                        <li key={tag} className={'inline-block bg-slate-50 py-1 px-3 border rounded mr-4 mb-3 text-slate-500'}>
                           <Icon type="tags" size={14} classes="mr-2" />
                           {tag}
                           <button className="ml-1 cursor-pointer rounded px-1 hover:bg-red-200 hover:text-red-700" onClick={() => removeTag(tag)}>
            ...
            {keyword && keyword.tags.length === 0 && (
               <div className="text-center w-full text-gray-500">
                  ...
                  <button className=" text-indigo-600" onClick={() => setShowAddTag(true)}>
                     + Add Tag
                  </button>
               </div>
            )}
         </div>
         {showAddTag && keyword && <AddTags existingTags={allTags} keywords={[keyword]} closeModal={() => setShowAddTag(false)} />}
      </Modal>
   );
};
