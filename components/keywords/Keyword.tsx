   ...
   const bestPosition: false | { position: number; date: string } = useMemo(() => {
      ...
      if (Object.keys(history).length > 0) {
         const historyArray = Object.keys(history)
            .map((itemID) => ({ date: itemID, position: history[itemID] }))
   ...
   return (
      ...
      >
         <div className=" w-3/4 font-semibold cursor-pointer lg:flex-1 lg:shrink-0 lg:basis-28 lg:w-auto lg:flex lg:items-center">
            ...
            {sticky && (
               <button className="ml-2 relative top-[2px]" title="Favorite">
                  <Icon type="star-filled" size={16} color="#fbd346" />
               </button>
            )}
         ...
         <div
            title={
               bestPosition && bestPosition.date
         ...
         >
            <a href={url} target="_blank" rel="noreferrer">
               <span className="mr-3 lg:hidden">
                  <Icon type="link-alt" size={14} color="#999" />
               </span>
               {turncatedURL || '-'}
            </a>
         </div>

         <div className="inline-block mt-[4] top-[-5px] relative lg:flex-1 lg:m-0 lg:top-0 max-w-[150px]">
            <span className="mr-2 lg:hidden">
               <Icon type="clock" size={14} color="#999" />
            </span>
         ...
         {showSCData && tableColumns.includes('Search Console') && (
            <div
               className="keyword_sc_data min-w-[170px] lg:max-w-[170px] text-xs mt-4 pt-2 border-t border-gray-100 top-[6px]
            ...
            >
               ...
               <span className="min-w-[40px]">
                  <span className="lg:hidden">Impressions: </span>
                  {keywordData?.scData?.impressions[scDataType as keyof KeywordSCDataChild] || 0}
               </span>
               <span className="min-w-[40px]">
                  <span className="lg:hidden">Visits: </span>
                  {keywordData?.scData?.visits[scDataType as keyof KeywordSCDataChild] || 0}
               </span>
            ...
            {showOptions && (
               <ul className="keyword_options customShadow absolute w-[180px] right-0 bg-white rounded border z-20">
                  <li>
                     ...
                     >
                        <span className=" bg-indigo-100 text-blue-700 px-1 rounded">
                           <Icon type="reload" size={11} />
                        </span>{' '}
                        Refresh Keyword
                     </a>
                  </li>
                  <li>
                     <a
                        className={optionsButtonStyle}
                        onClick={() => {
                           favoriteKeyword({ keywordID: ID, sticky: !sticky });
                           setShowOptions(false);
                        }}
                     >
                        ...
                        {sticky ? 'Unfavorite Keyword' : 'Favorite Keyword'}
                     </a>
                  </li>
                  <li>
                     <a
                     ...
                     >
                        <span className=" bg-green-100 text-green-500 px-1 rounded">
         ...
         {lastUpdateError && lastUpdateError.date && showPositionError && (
            <div
               className={`absolute p-2 bg-white z-30 border border-red-200 rounded w-[220px] left-4 shadow-sm text-xs 
            ${index > 2 ? 'lg:bottom-12 mt-[-70px]' : ' top-12'}`}
            >
               Error Updating Keyword position (Tried{' '}
               <TimeAgo title={dayjs(lastUpdateError.date).format('DD-MMM-YYYY, hh:mm:ss A')} date={lastUpdateError.date} />)
               ...
               <div className=" border-t-[1px] border-red-100 mt-2 pt-1">
                  {lastUpdateError.scraper && <strong className="capitalize">{lastUpdateError.scraper}: </strong>}
