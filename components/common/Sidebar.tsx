   ...
   return (
      <div className="sidebar pt-44 w-1/5 hidden lg:block" data-testid="sidebar">
                     ...
                     <Link href={`/domain/${d.slug}`} passHref={true}>
                        <a
                           className={`block cursor-pointer px-4 text-ellipsis max-w-[215px] overflow-hidden whitespace-nowrap rounded
                                        rounded-r-none ${
                                           `/domain/${d.slug}` === router.asPath ||
                                           `/domain/console/${d.slug}` === router.asPath ||
                                           `/domain/insight/${d.slug}` === router.asPath ||
                                           `/domain/ideas/${d.slug}` === router.asPath
                                              ? 'bg-white text-zinc-800 border border-r-0'
                                              : 'text-zinc-500'
                                        }`}
                        >
                           <img className={' inline-block mr-1'} src={`https://www.google.com/s2/favicons?domain=${d.domain}&sz=16`} alt={d.domain} />
                           {d.domain}
                        </a>
                     </Link>
                  </li>
         ...
         <div className="sidebar_add border-t font-semibold text-sm text-center mt-6 w-[80%] ml-3 text-zinc-500">
            <button data-testid="add_domain" onClick={() => showAddModal(true)} className="p-4 hover:text-blue-600">
               + Add Domain
            </button>
         </div>
      </div>
