            ...
            <div data-testid="domain_settings" className=" text-sm">
               ...
               >
                  <ul>
                     <li
                        className={`${tabStyle} ${
                           currentTab === 'notification' ? ' bg-white text-blue-600 border-slate-200' : 'border-transparent'
                        } `}
                     ...
                     <li
                        className={`${tabStyle} ${
                           currentTab === 'searchconsole' ? ' bg-white text-blue-600 border-slate-200' : 'border-transparent'
                        }`}
               ...
               {!isUpdating && settingsError?.msg && <div className="w-full mt-4 p-3 text-sm bg-red-50 text-red-700">{settingsError.msg}</div>}
            </div>

            <div className="flex justify-between border-t-[1px] border-gray-100 mt-8 pt-4 pb-0">
               <button className="text-sm font-semibold text-red-500" onClick={() => setShowRemoveDomain(true)}>
            ...
            <Modal closeModal={() => setShowRemoveDomain(false)} title={`Remove Domain ${domain.domain}`}>
               <div className="text-sm">
                  ...
                  <div className="mt-6 text-right font-semibold">
                     ...
                     <button className=" py-1 px-5 rounded cursor-pointer bg-red-400 text-white" onClick={() => deleteMutate(domain)}>
                        Remove
                     </button>
                  </div>
               </div>
