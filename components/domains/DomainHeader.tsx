      ...
      <div className="domain_kewywords_head w-full ">
         <div>
            <h1 className="hidden lg:block text-xl font-bold my-3" data-testid="domain-header">
               {domain && domain.domain && (
                  <>
                     <i className=" capitalize font-bold not-italic">{domain.domain.charAt(0)}</i>
               ...
               <SelectField
                  options={
                     domains && domains.length > 0
                        ? domains.map((d) => {
                             return { label: d.domain, value: d.slug };
                          })
                        : []
                  }
         ...
         <div className="flex w-full justify-between mt-4 lg:mt-0">
            ...
            lg:z-auto lg:relative lg:mt-0 lg:border-0 lg:w-auto lg:bg-transparent`}
               ...
               >
                  {!isInsight && (
                     <button className={`domheader_action_button relative ${buttonStyle}`} aria-pressed="false" onClick={() => exportCsv()}>
                        <Icon type="download" size={20} />
                        <i className={`${buttonLabelStyle}`}>Export as csv</i>
                     </button>
                  )}
                  {!isConsole && !isInsight && !isIdeas && (
                     ...
                     >
                        ...
                        <i className={`${buttonLabelStyle}`}>Reload All Serps</i>
                     </button>
                  )}
                  <button
               ...
               {!isConsole && !isInsight && !isIdeas && (
                  ...
                  >
                     <span className="text-center leading-4 mr-2 inline-block rounded-full w-7 h-7 pt-1 bg-blue-700 text-white font-bold text-lg">
                        +
                     </span>
