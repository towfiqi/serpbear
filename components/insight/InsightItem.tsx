...
const InsightItem = ({ item, lastItem, type, domain }: InsightItemProps) => {
   ...
   if (type === 'countries') {
      firstItem = countries[country] && countries[country][0];
   }
   ...
   return (
      ...
      >
         <div className=" w-3/4 lg:flex-1 lg:basis-20 lg:w-auto font-semibold">
            ...
            {type === 'pages' && domain ? (
               <a href={`https://${domain}${page}`} target="_blank" rel="noreferrer">
                  {firstItem}
               </a>
            ) : (
               firstItem
         ...
         <div
            className={`keyword_position absolute bg-[#f8f9ff] w-fit min-w-[50px] h-14 p-2 text-base mt-[-55px] rounded right-5 lg:relative
