...
const AdWordsSettings = ({ settings, settingsError, updateSettings, performUpdate, closeSettings }: AdWordsSettingsProps) => {
   ...
   const udpateAndAuthenticate = () => {
      if (adwords_client_id && adwords_client_secret) {
         ...
         link.href = `https://accounts.google.com/o/oauth2/v2/auth/oauthchooseaccount?access_type=offline&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fadwords&response_type=code&client_id=${adwords_client_id}&redirect_uri=${`${encodeURIComponent(
            window.location.origin,
         )}/api/adwords`}&service=lso&o2v=2&theme=glif&flowName=GeneralOAuthFlow`;
         link.target = '_blank';
         link.click();
         if (performUpdate) {
            performUpdate();
