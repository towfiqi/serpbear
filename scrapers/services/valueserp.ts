...
const valueSerp: ScraperSettings = {
   ...
   scrapeURL: (keyword, settings, countryData) => {
      ...
      console.log(
         `https://api.valueserp.com/search?api_key=${settings.scaping_api}&q=${encodeURIComponent(
            keyword.keyword,
         )}&gl=${country}&hl=${lang}${device}${location}&num=100&output=json&include_answer_box=false&include_advertiser_info=false`,
      );
      return `https://api.valueserp.com/search?api_key=${settings.scaping_api}&q=${encodeURIComponent(
         keyword.keyword,
      )}&gl=${country}&hl=${lang}${device}${location}&num=100&output=json&include_answer_box=false&include_advertiser_info=false`;
   },
