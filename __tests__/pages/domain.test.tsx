...
describe('SingleDomain Page', () => {
   ...
   beforeEach(() => {
      ...
      useUpdateSettingsFunc.mockImplementation(() => ({ mutate: jest.fn() }));
   ...
   it('Country Filter should function properly', async () => {
      ...
      // Select a specific country (BD) which should filter to 1 keyword
      ...
      const keywordsCount = document.querySelectorAll('.keyword').length;
      // Filtering by a specific country shows only keywords from that country
      expect(keywordsCount).toBe(1);
   });
