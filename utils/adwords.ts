...
export const getAdwordsKeywordIdeas = async (credentials:AdwordsCredentials, adwordsDomainOptions:IdeaSettings, test:boolean = false) => {
   ...
   if (accessToken) {
      ...
      try {
         // API: https://developers.google.com/google-ads/api/rest/reference/rest/v22/customers/generateKeywordIdeas
         ...
         const resp = await fetch(`https://googleads.googleapis.com/v22/customers/${customerID}:generateKeywordIdeas`, {
            method: 'POST',
            headers: {
   ...
   if (accessToken) {
      ...
      for (const country in keywordRequests) {
         if (Object.hasOwn(keywordRequests, country) && keywordRequests[country].length > 0) {
            try {
               // API: https://developers.google.com/google-ads/api/rest/reference/rest/v22/customers/generateKeywordHistoricalMetrics
               const customerID = account_id.replaceAll('-', '');
               const geoTargetConstants = countries[country][3]; // '2840';
               ...
               const resp = await fetch(`https://googleads.googleapis.com/v22/customers/${customerID}:generateKeywordHistoricalMetrics`, {
                  method: 'POST',
                  headers: {
                     'Content-Type': 'application/json',
                     'developer-token': developer_token,
